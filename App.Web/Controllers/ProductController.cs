        ...
        public async Task<IActionResult> ProductDetail(int id)
        {
            var product = await _repository.GetOneAsync<AppProduct, ProductDetailClientVM>(id, p => new ProductDetailClientVM
            {
                ...
                MstProductColors = p.AppProductDetails
                        ...
                        .Distinct()
                        .ToList(),
                RamOptions = p.AppProductDetails
                        .Select(x => x.Ram).Distinct().OrderByDescending(x => x).ToList(),
                RomOptions = p.AppProductDetails
                        .Select(x => x.Rom).Distinct().OrderBy(x => x).ToList()
            });
