...
namespace App.Web.WebConfig
{
	public class AutoMapperProfile : Profile
	{
		...
		//		.ForMember(item => item.CountProduct, opts => opts.MapFrom(entity => entity.AppProducts == null ? 0 : entity.AppProducts.Where(x => x.DeletedDate == null).Count()));
		//});
	...
	//	{
	//		mapper.CreateMap<AppProduct, ProductDetailVM>()
	//			.ForMember(x => x.CategoryName, opt => opt.MapFrom(e => e.ProductCategory == null ? "" : (e.ProductCategory.ParentCategory == null ? e.ProductCategory.Name : $"[{e.ProductCategory.ParentCategory.Name}] > [{e.ProductCategory.Name}]")))
	//			.ForMember(x => x.ColorName, opt => opt.MapFrom(u => u.ProductColor.ColorName))
	//			.ReverseMap();
